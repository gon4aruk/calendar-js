...
const createEventElement = eventItem => {
  ...
  const startMinutes = getDateTime(eventItem.date, eventItem.startTime).getMinutes();
  ...
  const timeOfStart = getDateTime(eventItem.date, eventItem.startTime).toString().slice(16, 21);
  const timeOfEnd = getDateTime(eventItem.date, eventItem.endTime).toString().slice(16, 21);
...
const filterEventsList = eventsList => {
  return eventsList.filter(elem => {
    ...
    return (
      ...
      startTime.getTime() < shmoment(dateOfMonday).add('days', 7).result().getTime()
    );
  });
};
...
const appendEvents = eventsListOnThisWeek => {
  eventsListOnThisWeek.forEach(element => {
    ...
    const timeSlot = document
      ...
      .querySelector(`.calendar__time-slot[data-time = '${startTime.getHours()}']`);
